...
def test_register_one_airquality():
    response = requests.post(
        "http://localhost:5000/register_one",
...
def test_register_one_uv():
    response = requests.post(
        "http://localhost:5000/register_one",
        json={"timestamp": 1, "type_of_sensor": "ULTRAVIOLETRADIATION", "read": 5.0},
    )
    assert response.json()["result"] == "OK"
...
def test_register_many_airquality():
    response = requests.post(
        "http://localhost:5000/register_many",
        json={
            "readings": [
...
def test_highest_accumulated_airquality():
    response = requests.get(
        "http://localhost:5000/highest_accumulated",
        params={"type_of_sensor": "AIRQUALITY"},
    )
...
def test_highest_accumulated_uv():
    response = requests.get(
        "http://localhost:5000/highest_accumulated",
        params={"type_of_sensor": "ULTRAVIOLETRADIATION"},
    )
    result = response.json()
